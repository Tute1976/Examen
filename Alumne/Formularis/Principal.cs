...
namespace Examen.Alumne.Formularis
{
    public partial class Principal : MetroForm
    {
        ...
        private void TimerTemps_Tick(object sender, EventArgs e)
        {
            try
            {
                var json = ClientTcp.EnviarEstat(AdreçaPortProfessor, EstacioAlumne, TipusMissatge.Temps, out var pitar, out var bloquejar, out var aturar);
                Aplicacions = json.Deserialitzar<List<Aplicacio>>();
                ...
                foreach (var aplicacio in Aplicacions.Where(aplicacio => aplicacio.EnExecucio()))
                {
                    ...
                    ClientTcp.EnviarEstat(AdreçaPortProfessor, EstacioAlumne, TipusMissatge.Deteccio, out _, out _, out _, $"{aplicacio.Nom}:{aturada.SiNo()}");
                }
            }
            catch
            {
